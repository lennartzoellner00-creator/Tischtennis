...
function ergaenzeRunden(runden, numSpieler) {
  ...
  // Berechne korrekte Werte aus der Spieleranzahl (nicht aus korrupten Firebase-Daten)
  const size        = nextPow2(numSpieler);
  const r0Expected  = size / 2;             // z.B. 17 Spieler → size=32 → r0 hat 16 Matches
  const totalRunden = Math.log2(size);      // z.B. 17 → 5 Runden

  // r0 auf korrekte Länge auffüllen falls Firebase Einträge gelöscht hat
  while (runden[0].length < r0Expected) runden[0].push(leerMatch());

  // Fehlende Folgerunden anhängen
  let cur = runden[runden.length - 1].length;
  while (runden.length < totalRunden) {
    cur = Math.ceil(cur / 2);
    const nr = [];
    for (let i = 0; i < cur; i++) nr.push(leerMatch());
    runden.push(nr);
  }
  return runden;
}
      ...
      if (isBye) {
        html += `<div class="bracket-match bye-match">
          <div class="bracket-slot bye-slot">
            ...
            <span class="bracket-slot-bye">FREILOS</span>
          </div>
        </div>`;
        return;
      }
...
function roundName(displayIdx, total) {
  // Abstand zum Finale bestimmt den Namen
  const rem = total - displayIdx;
  if (rem === 1) return "FINALE";
  if (rem === 2) return "HALBFINALE";
  if (rem === 3) return "VIERTELFINALE";
  if (rem === 4) return "ACHTELFINALE";
  if (rem === 5) return "RUNDE DER 32";
  if (rem === 6) return "RUNDE DER 64";
  return "VORRUNDE";
}
