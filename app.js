...
function renderVerlauf() {
  ...
  try {
    // Reguläre Spiele — mit Index für Edit
    ...
    log.innerHTML = allSpiele.map(sp => {
      ...
      const editBtn = !sp.isTurnier
        ? `<button class="log-edit-btn" onclick="adminEditSpiel(${sp.idx})">✏️</button>` : "";
      return `<div class="log-item ${sp.isTurnier ? "log-turnier" : ""}">
        ...
        <div style="display:flex;align-items:center;gap:8px">${delta}${editBtn}</div>
      </div>`;
    }).join("");
  } catch (err) {
...
// VORSCHAU
// ════════════════════════════════════════════════════════════════════
window.adminEditSpiel = async function(idx) {
  ...
  showModal(`
    ...
    <div style="display:flex;gap:8px;align-items:center;margin-top:6px">
      ...
      <input type="number" id="pmScoreA" class="pin-input" style="width:60px;text-align:center" placeholder="0" value="${sp.score_sieger ?? ""}"/>
      <span style="color:var(--muted)">:</span>
      <input type="number" id="pmScoreB" class="pin-input" style="width:60px;text-align:center" placeholder="0" value="${sp.score_verlierer ?? ""}"/>
  ...
  document.getElementById("pmEditBtn").onclick = async () => {
    ...
    if (siegerKey && sp.elo_sieger_vorher != null) {
      daten.spieler[siegerKey].elo     = sp.elo_sieger_vorher;
      daten.spieler[siegerKey].siege   = Math.max(0, (daten.spieler[siegerKey].siege || 1) - 1);
      daten.spieler[siegerKey].spiele  = Math.max(0, (daten.spieler[siegerKey].spiele || 1) - 1);
    }
    if (verliererKey && sp.elo_verlierer_vorher != null) {
      daten.spieler[verliererKey].elo          = sp.elo_verlierer_vorher;
      daten.spieler[verliererKey].niederlagen  = Math.max(0, (daten.spieler[verliererKey].niederlagen || 1) - 1);
      daten.spieler[verliererKey].spiele       = Math.max(0, (daten.spieler[verliererKey].spiele || 1) - 1);
    }

    // Spiel entfernen und neu berechnen
    daten.spiele.splice(idx, 1);
    closeModal();
    if (siegerKey && verliererKey) {
      doSubmitMatch(siegerKey, verliererKey, sA, sB);
    } else {
      speichereDaten();
      renderRangliste(); renderPlayerList(); renderVerlauf();
    }
  };
};
  ...
  runden.forEach((runde, ri) => {
    ...
    runde.forEach((m, mi) => {
      ...
      const clickHandler = isPlayable
        ...
        ? `onclick="adminEditBracketMatch('${type}',${ri},${mi})" style="cursor:pointer"`
        : "";
