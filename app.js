...
function renderBracketHTML(runden, container, type) {
  ...
  echteRunden.forEach((runde, displayIdx) => {
    ...
    html += `<div class="bracket-round">
      <div class="bracket-round-title">${roundName(runde.length)}</div>`;
...
function roundName(totalMatches) {
  // totalMatches = Gesamtanzahl Slots inkl. Freilose
  // Benennung nach Anzahl der Spieler die IN diese Runde eintreten (totalMatches * 2)
  if (totalMatches === 1)  return "FINALE";
  if (totalMatches === 2)  return "HALBFINALE";
  if (totalMatches === 4)  return "VIERTELFINALE";
  if (totalMatches === 8)  return "ACHTELFINALE";
  if (totalMatches === 16) return "RUNDE DER 32";
  if (totalMatches === 32) return "RUNDE DER 64";
  return "VORRUNDE";
}
