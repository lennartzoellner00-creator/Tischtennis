...
function renderBracketHTML(runden, container, type) {
  // MATCH_H: feste Höhe eines Match-Blocks in px (muss mit CSS .bracket-match height übereinstimmen)
  const MATCH_H = 72;
  ...
  runden.forEach((runde, ri) => {
    ...
    // In Runde ri müssen je 2^ri Matches "Platz" für 1 Eintrag in der nächsten Runde lassen
    const topPad  = MATCH_H / 2;  // Erste Runde leicht einrücken

    html += `<div class="bracket-round">
      <div class="bracket-round-title">${label}</div>
      <div class="bracket-matches" style="padding-top:${topPad}px;">`;
    ...
    runde.forEach((m, mi) => {
      ...
      // Abstand nach jedem Match: klein innerhalb eines Paares, groß zwischen Paaren
      const afterPair  = MATCH_H * Math.pow(2, ri);
      const mb = (mi % 2 === 1 && mi < runde.length - 1) ? `margin-bottom:${afterPair}px` : `margin-bottom:4px`;

      if (isBye) {
        html += `<div class="bracket-match done-match" style="${mb}">
          <div class="bracket-slot winner-slot">
            <span class="bracket-slot-name">${spielerName(m.sieger)}</span>
            <span class="bracket-slot-score"></span>
          </div>
          <div class="bracket-slot bye-slot">
            <span class="bracket-slot-name" style="opacity:0.4;font-style:italic">FREILOS</span>
            <span class="bracket-slot-score"></span>
          </div>
        </div>`;
        return;
      ...
      html += `<div class="${matchClass}" ${clickHandler} style="${mb}">
        <div class="${slotA}">
