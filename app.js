...
// Firebase gibt verschachtelte Arrays als Objekte zurueck — diese Funktion
// wandelt alle Ebenen korrekt um
function toArr(val) {
  if (!val) return [];
...
function leerMatch() {
  return { a: null, b: null, sieger: null, verlierer: null, scoreW: null, scoreL: null, datum: null };
}

// Firebase speichert Arrays als sparse Objekte — fehlende Indizes werden geloescht.
// Diese Funktion stellt die korrekten Indizes wieder her.
function toArrSparse(val) {
  if (!val) return [];
  if (Array.isArray(val)) return val;
  const numKeys = Object.keys(val).map(Number).filter(k => !isNaN(k));
  if (numKeys.length === 0) return Object.values(val);
  const maxKey = Math.max(...numKeys);
  const arr = [];
  for (let i = 0; i <= maxKey; i++) {
    arr.push(val[i] ? val[i] : leerMatch());
  }
  return arr;
}

function ergaenzeRunden(runden, numSpieler) {
  if (!runden || runden.length === 0) return runden;
  const size        = nextPow2(numSpieler);
  const totalRunden = Math.log2(size);
  // Jede Runde ri muss exakt size/2^(ri+1) Matches haben
  for (let ri = 0; ri < totalRunden; ri++) {
    const expected = size / Math.pow(2, ri + 1);
    if (!runden[ri]) runden[ri] = [];
    while (runden[ri].length < expected) runden[ri].push(leerMatch());
  }
  return runden;
}


function fixTurnier(t) {
  if (!t) return t;
  if (t.teilnehmer) t.teilnehmer = toArr(t.teilnehmer);
  if (t.hauptrunden) {
    const n = t.bracketN || t.teilnehmer.length;
    t.hauptrunden = ergaenzeRunden(toArrSparse(t.hauptrunden).map(r => toArrSparse(r)), n);
  }
  if (t.trostrunden) {
    const n = t.trostBracketN || (t.hauptrunden ? t.hauptrunden[0].filter(m => m.a && m.b).length * 2 : 2);
    t.trostrunden = ergaenzeRunden(toArrSparse(t.trostrunden).map(r => toArrSparse(r)), Math.max(n, 2));
  }
  if (t.gruppen) {
    t.gruppen = toArr(t.gruppen).map(g => {
      if (g.spiele) g.spiele = toArr(g.spiele);
      if (g.spieler) g.spieler = toArr(g.spieler);
      return g;
    });
  }
  return t;
...
function onFirebaseDaten(snapshot) {
  ...
  if (remote) {
    ...
    daten.spiele    = toArr(daten.spiele);
    daten.turniere  = toArr(daten.turniere).map(fixTurnier);
    daten.eloEvents = toArr(daten.eloEvents);
...
function renderVerlauf() {
  ...
  try {
    // Reguläre Spiele — ts als stabiler Key für Edit
    const allSpiele = daten.spiele.map((sp, idx) => ({
      ts: sp.ts || idx,
      datum: sp.datum,
      sieger: sp.sieger,
    ...
    log.innerHTML = allSpiele.map(sp => {
      ...
      const editBtn = `<button class="log-edit-btn" onclick="adminEditSpiel(${sp.ts_key},${sp.isTurnier})">✏️</button>`;
...
// VORSCHAU
// ════════════════════════════════════════════════════════════════════
// Berechnet alle ELO-Werte von Grund auf neu anhand der gespeicherten Spiele
function allesNeuBerechnen() {
  // 1. Alle Spieler auf Startwerte zurücksetzen
  Object.keys(daten.spieler).forEach(k => {
    daten.spieler[k].elo           = START_ELO;
    daten.spieler[k].siege         = 0;
    daten.spieler[k].niederlagen   = 0;
    daten.spieler[k].spiele        = 0;
    daten.spieler[k].punktdifferenz = 0;
    daten.spieler[k].gesamtPunkte  = 0;
    daten.spieler[k].gesamtSpiele  = 0;
    daten.spieler[k].turnierPunkte = 0;
  });

  // 2. Alle Spiele chronologisch sortieren und neu berechnen
  daten.spiele.sort((a, b) => (a.ts || 0) - (b.ts || 0));
  daten.spiele.forEach(sp => {
    const siegerKey    = Object.keys(daten.spieler).find(k => daten.spieler[k].name === sp.sieger);
    const verliererKey = Object.keys(daten.spieler).find(k => daten.spieler[k].name === sp.verlierer);
    if (!siegerKey || !verliererKey) return;

    const s = daten.spieler[siegerKey];
    const v = daten.spieler[verliererKey];
    const eloSOld = s.elo, eloVOld = v.elo;
    const erw = erwartet(eloSOld, eloVOld);
    const diff = (sp.score_sieger || 0) - (sp.score_verlierer || 0);

    s.elo = neueElo(eloSOld, erw, 1);
    v.elo = neueElo(eloVOld, 1 - erw, 0);
    s.siege         += 1; s.spiele        += 1;
    v.niederlagen   += 1; v.spiele        += 1;
    s.punktdifferenz += diff; v.punktdifferenz -= diff;
    s.gesamtPunkte  += (sp.score_sieger || 0);
    v.gesamtPunkte  += (sp.score_verlierer || 0);
    s.gesamtSpiele  += 1; v.gesamtSpiele  += 1;

    // ELO-Werte im Spieleintrag aktualisieren
    sp.elo_sieger_vorher    = eloSOld;
    sp.elo_verlierer_vorher = eloVOld;
    sp.elo_sieger_nachher   = s.elo;
    sp.elo_verlierer_nachher = v.elo;
  });

  // 3. Turnier-Punkte neu vergeben
  daten.eloEvents = [];
  daten.turniere.forEach(t => {
    if (t.status !== "abgeschlossen" || !t.mitWertung || !t.sieger) return;
    const [p1, p2, p3] = getPrizes(t.teilnehmer.length);
    [[t.sieger, p1], [t.platz2, p2], [t.platz3, p3]].forEach(([k, pts]) => {
      if (!k || !daten.spieler[k] || !pts) return;
      daten.spieler[k].turnierPunkte = (daten.spieler[k].turnierPunkte || 0) + pts;
      daten.spieler[k].elo = Math.round((daten.spieler[k].elo + pts) * 10) / 10;
    });
  });
}

window.adminEditSpiel = async function(ts) {
  const idx = daten.spiele.findIndex(s => s.ts === ts);
  const sp  = daten.spiele[idx];
  if (!sp) return;
  showModal(`
    <div class="pin-modal-title">✏️ Spiel bearbeiten</div>
    <div class="pin-modal-sub">${sp.sieger} vs ${sp.verlierer}</div>
    <label class="pin-field-label">Admin-PIN</label>
    <input type="password" id="pmAdminPin" class="pin-input" placeholder="Admin-PIN …" maxlength="20" autofocus/>
    <label class="pin-field-label" style="margin-top:12px">Neues Ergebnis</label>
    <div style="display:flex;gap:8px;align-items:center;margin-top:6px">
      <span style="font-size:.85rem;color:var(--text);flex:1">${sp.sieger}</span>
      <input type="number" id="pmScoreA" class="pin-input" style="width:60px;text-align:center" value="${sp.score_sieger ?? ""}"/>
      <span style="color:var(--muted)">:</span>
      <input type="number" id="pmScoreB" class="pin-input" style="width:60px;text-align:center" value="${sp.score_verlierer ?? ""}"/>
      <span style="font-size:.85rem;color:var(--text);flex:1;text-align:right">${sp.verlierer}</span>
    </div>
    <button class="pin-btn" id="pmEditBtn" style="margin-top:16px">Speichern</button>
    <div id="pmEditErr" style="color:var(--red);font-size:.75rem;margin-top:8px;min-height:16px"></div>
  `);
  document.getElementById("pmEditBtn").onclick = async () => {
    const pin   = document.getElementById("pmAdminPin").value;
    const sA    = parseInt(document.getElementById("pmScoreA").value);
    const sB    = parseInt(document.getElementById("pmScoreB").value);
    const errEl = document.getElementById("pmEditErr");
    if (!await pinKorrekt(pin, daten.pins.admin)) {
      errEl.textContent = "❌ Falscher Admin-PIN.";
      document.getElementById("pmAdminPin").value = "";
      return;
    }
    if (isNaN(sA) || isNaN(sB) || sA < 0 || sB < 0) { errEl.textContent = "❌ Ungültige Scores."; return; }
    if (sA === sB) { errEl.textContent = "❌ Unentschieden nicht möglich."; return; }

    // Sieger/Verlierer ggf. tauschen
    const neuerSieger    = sA > sB ? sp.sieger    : sp.verlierer;
    const neuerVerlierer = sA > sB ? sp.verlierer : sp.sieger;
    sp.sieger          = neuerSieger;
    sp.verlierer       = neuerVerlierer;
    sp.score_sieger    = Math.max(sA, sB);
    sp.score_verlierer = Math.min(sA, sB);

    allesNeuBerechnen();
    closeModal();
    speichereDaten();
    fuelleSelects(); renderRangliste(); renderPlayerList(); renderVerlauf(); updateHeaderStats();
    showToast("matchToast", "✅ Spiel bearbeitet — ELO neu berechnet.");
  };
};

window.adminEditBracketMatch = async function(type, ri, mi) {
  const t = daten.turniere[activeTurnierIdx];
  if (!t) return;
  const runden = type === "haupt" ? t.hauptrunden : t.trostrunden;
  const m = runden[ri][mi];
  if (!m || !m.sieger || !m.verlierer) return;

  showModal(`
    <div class="pin-modal-title">✏️ Turnierspiel bearbeiten</div>
    <div class="pin-modal-sub">${spielerName(m.a)} vs ${spielerName(m.b)}</div>
    <label class="pin-field-label">Admin-PIN</label>
    <input type="password" id="pmAdminPin2" class="pin-input" placeholder="Admin-PIN …" maxlength="20" autofocus/>
    <label class="pin-field-label" style="margin-top:12px">Neues Ergebnis</label>
    <div style="display:flex;gap:8px;align-items:center;margin-top:6px">
      <span style="font-size:.85rem;color:var(--text);flex:1">${spielerName(m.a)}</span>
      <input type="number" id="pmScoreA2" class="pin-input" style="width:60px;text-align:center" placeholder="0" value="${m.sieger===m.a ? m.scoreW : m.scoreL}"/>
      <span style="color:var(--muted)">:</span>
      <input type="number" id="pmScoreB2" class="pin-input" style="width:60px;text-align:center" placeholder="0" value="${m.sieger===m.b ? m.scoreW : m.scoreL}"/>
      <span style="font-size:.85rem;color:var(--text);flex:1;text-align:right">${spielerName(m.b)}</span>
    </div>
    <button class="pin-btn" id="pmEditBtn2" style="margin-top:16px">Speichern</button>
    <div id="pmEditErr2" style="color:var(--red);font-size:.75rem;margin-top:8px;min-height:16px"></div>
  `);
  document.getElementById("pmEditBtn2").onclick = async () => {
    const pin   = document.getElementById("pmAdminPin2").value;
    const sA    = parseInt(document.getElementById("pmScoreA2").value);
    const sB    = parseInt(document.getElementById("pmScoreB2").value);
    const errEl = document.getElementById("pmEditErr2");
    if (!await pinKorrekt(pin, daten.pins.admin)) {
      errEl.textContent = "❌ Falscher Admin-PIN.";
      document.getElementById("pmAdminPin2").value = "";
      return;
    }
    if (isNaN(sA) || isNaN(sB) || sA < 0 || sB < 0) { errEl.textContent = "❌ Ungültige Scores."; return; }
    if (sA === sB) { errEl.textContent = "❌ Unentschieden nicht möglich."; return; }

    const sieger    = sA > sB ? m.a : m.b;
    const verlierer = sA > sB ? m.b : m.a;
    const scoreW    = Math.max(sA, sB);
    const scoreL    = Math.min(sA, sB);

    m.sieger    = sieger;
    m.verlierer = verlierer;
    m.scoreW    = scoreW;
    m.scoreL    = scoreL;

    // Sieger in nächste Runde propagieren
    vollPropagieren(runden);

    // Podest neu berechnen falls Turnier abgeschlossen
    if (t.phase === "fertig") {
      const finalRunde = t.hauptrunden[t.hauptrunden.length - 1];
      const finalMatch = finalRunde[0];
      if (finalMatch && finalMatch.sieger) {
        // ELO-Bonus vom alten Ergebnis entfernen
        if (t.mitWertung) {
          const [p1, p2, p3] = getPrizes(t.teilnehmer.length);
          [[t.sieger, p1], [t.platz2, p2], [t.platz3, p3]].forEach(([k, pts]) => {
            if (!k || !daten.spieler[k] || !pts) return;
            daten.spieler[k].elo = Math.round((daten.spieler[k].elo - pts) * 10) / 10;
            daten.spieler[k].turnierPunkte = Math.max(0, (daten.spieler[k].turnierPunkte || 0) - pts);
          });
        }
        // Neues Podest setzen
        t.sieger = finalMatch.sieger;
        t.platz2 = finalMatch.verlierer;
        t.platz3 = t.dritterPlatzSpiel?.sieger || null;
        // ELO-Bonus neu vergeben
        if (t.mitWertung) {
          const [p1, p2, p3] = getPrizes(t.teilnehmer.length);
          [[t.sieger, p1], [t.platz2, p2], [t.platz3, p3]].forEach(([k, pts]) => {
            if (!k || !daten.spieler[k] || !pts) return;
    ...
    showToast("matchToast", "✅ Turnierspiel bearbeitet.");
  };
};
...
function renderHauptrunde(t) {
  ...
  if (t.phase === "fertig") {
    ...
    // Bracket trotzdem rendern damit abgeschlossene Spiele bearbeitet werden können
    const hauptEl = document.getElementById("bracketHaupt");
    const trostEl = document.getElementById("bracketTrost");
    if (hauptEl && t.hauptrunden) renderBracketHTML(t.hauptrunden, hauptEl, "haupt");
    if (trostEl && t.trostrunden) renderBracketHTML(t.trostrunden, trostEl, "trost");
    return;
  }
...
function renderBracketHTML(runden, container, type) {
  const MATCH_H = 72;
  const G       = 12;               // Basis-Gap in Runde 0
  const U       = MATCH_H + G;      // 84px pro Slot

  let html = `<div class="bracket">`;
  runden.forEach((runde, ri) => {
    const label  = roundName(ri, runden.length);
    const topPad = (Math.pow(2, ri) - 1) * U / 2;
    const gap    = ri === 0 ? G : Math.pow(2, ri) * U - MATCH_H;

    html += `<div class="bracket-round">
      <div class="bracket-round-title">${label}</div>
      <div class="bracket-matches" style="padding-top:${topPad}px;gap:${gap}px;">`;
    ...
    runde.forEach((m, mi) => {
      ...
      if (isBye) {
        // Gleiche Hoehe wie normales Match (MATCH_H) damit gap-Berechnung stimmt
        html += `<div class="bracket-match bye-match">
          <div class="bracket-slot winner-slot">
            <span class="bracket-slot-name">${spielerName(m.sieger)}</span>
          </div>
          <div class="bracket-slot bye-slot">
            <span class="bracket-slot-name" style="opacity:0.35;font-style:italic">FREILOS</span>
          </div>
        </div>`;
        return;
      }
      ...
      const aWon   = m.sieger === m.a && m.verlierer;
      const bWon   = m.sieger === m.b && m.verlierer;
      const scoreA = aWon ? `${m.scoreW}` : bWon ? `${m.scoreL}` : "";
      const scoreB = bWon ? `${m.scoreW}` : aWon ? `${m.scoreL}` : "";

      const matchClass = isSelected ? "bracket-match selected-match"
                       : isPlayable ? "bracket-match playable-match"
                       : (!m.a && !m.b) ? "bracket-match pending-match"
                       :                  "bracket-match done-match";
      const clickHandler = isPlayable
        ? `onclick="selectBracketMatch('${type}',${ri},${mi})"`
        : (m.sieger && m.verlierer)
        ? `onclick="adminEditBracketMatch('${type}',${ri},${mi})"`
        : "";
      const slotA = aWon ? "bracket-slot winner-slot" : bWon ? "bracket-slot loser-slot" : "bracket-slot";
      const slotB = bWon ? "bracket-slot winner-slot" : aWon ? "bracket-slot loser-slot" : "bracket-slot";
...
function roundName(ri, totalRunden) {
  const rem = totalRunden - ri;
  if (rem === 1) return "FINALE";
  if (rem === 2) return "HALBFINALE";
  if (rem === 3) return "VIERTELFINALE";
  if (rem === 4) return "ACHTELFINALE";
  if (rem === 5) return "SECHZEHNTELFINALE";
  return "VORRUNDE";
}
