...
function fixTurnier(t) {
  if (!t) return t;
  // hauptrunden: Array von Runden, jede Runde ein Array von Matches
  if (t.hauptrunden) {
    t.hauptrunden = toArr(t.hauptrunden).map(r => toArr(r));
  }
  if (t.trostrunden) {
    t.trostrunden = toArr(t.trostrunden).map(r => toArr(r));
  }
  if (t.gruppen) {
...
function renderBracketHTML(runden, container, type) {
  // Nur Runden mit echten Spielen zaehlen (fuer korrekte Labels)
  const echteRunden = runden.filter(r => r.some(m => m.a && m.b));

  let html = `<div class="bracket">`;
  echteRunden.forEach((runde, displayIdx) => {
    const ri          = runden.indexOf(runde);
    const echteSpiele = runde.filter(m => m.a && m.b);
    const freilose    = runde.filter(m => m.sieger && !m.verlierer).length;

    html += `<div class="bracket-round">
      <div class="bracket-round-title">${roundName(displayIdx, echteRunden.length)}</div>`;

    if (freilose > 0) {
      html += `<div class="bye-note">${freilose} Spieler mit Freilos weiter</div>`;
    }

    html += `<div class="bracket-matches">`;

    echteSpiele.forEach((m) => {
      const mi         = runde.indexOf(m);
      ...
      const slotClassA = aWon ? "bracket-slot winner-slot" :
                         bWon ? "bracket-slot loser-slot" : "bracket-slot";
      const slotClassB = bWon ? "bracket-slot winner-slot" :
                         aWon ? "bracket-slot loser-slot" : "bracket-slot";
    ...
    html += `</div></div>`;
    if (displayIdx < echteRunden.length - 1) html += `<div style="width:16px;flex-shrink:0"></div>`;
  });
  html += `</div>`;
